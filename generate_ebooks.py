...
Formats:
    ...
    pdf   Built directly from the extracted chapter HTML (no pandoc/epub/
          ...
          print-to-PDF can only output ONE page size per PDF (even though
          Paged.js itself lays each page out at its own size correctly),
          the two parts are rendered as separate PDFs and then joined at
...
def run(fmt, ext, build_one, setup=None, test_limit=5, args=None):
    ...
    test_limit: cap on how many pending books to build per run. Set to
              None to build everything. Overridden by args.all / args.limit
              when args is provided.
    args: pre-parsed argparse.Namespace (see main()). If not provided,
              this function parses sys.argv itself (kept for standalone
              use/testing).
    """
    if args is None:
    ...
    # skipped -- no separate published-files list needed since the output
    # folder itself, checked out fresh by CI, *is* that list.
    existing_names = {
        fn for fn in os.listdir(out_dir) if fn.lower().endswith(f".{ext}")
    }
    if existing_names:
        print(f"Found {len(existing_names)} already-built {ext} file(s) in {out_dir}/, will skip those")
...
# =====================================================================
# EPUB format (formerly generate_epub.py)
# =====================================================================

def build_one_epub(book, html_files, cover_path, work_dir, extract_dir, out_dir, css_path, ctx):
    out_path = os.path.join(out_dir, f"{book['slug']}.epub")
    build_epub(book, html_files, cover_path, css_path, out_path, extract_dir=extract_dir)
    return out_path
...
def _pdf_html_head(title, css_path, page_css):
    ...
    return (
        ...
        # Injected here (not in style.css) because Chromium's print-to-PDF
        # only honors one @page rule per render -- see the note at the top
        # of style.css for why.
...
def render_pdf(html_path, out_path):
    """Hands the assembled HTML off to render_pdf.js, which uses
...
def build_one_pdf(book, html_files, cover_path, work_dir, extract_dir, out_dir, css_path, ctx):
    slug = book["slug"]
...
# =====================================================================
# Entry point: pick the format, then hand off to the shared run() loop.
# =====================================================================
...
def _add_common_args(ap):
    ap.add_argument(
        "--all",
        action="store_true",
        help="Build every pending book, ignoring the demo/test limit.",
    )
    ap.add_argument(
        "--limit",
        type=int,
        default=None,
        help=(
            "Cap on how many pending books to build. Overrides the "
            "script's default test limit. Ignored if --all is passed."
        ),
    )
...
def main():
    ...
    ap.add_argument(
        ...
        required=True,
        help="Which ebook format to build.",
    )
    _add_common_args(ap)
    args = ap.parse_args()

    cfg = FORMATS[args.format]
    run(args.format, args.format, cfg["build_one"], setup=cfg["setup"], test_limit=5, args=args)
